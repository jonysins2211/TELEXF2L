...
ADMINS = list(map(int, environ.get('ADMINS', '982105601').split()))  # List of admin user IDs
...
# 🌐 Server Settings
PORT = int(getenv('PORT', '8087'))  # Port for web server
...
FQDN = getenv("FQDN", "https://telexoriginals-f2l-bot-5cd151ae5e5d.herokuapp.com/") or BIND_ADDRESS  # Full domain name or fallback to bind address
PORT_SEGMENT = "" if NO_PORT else f":{PORT}/"  # Port in URL if not disabled
PROTOCOL = "https" if HAS_SSL else "http"  # Protocol for URL
URL = f"https://telexoriginals-f2l-bot-5cd151ae5e5d.herokuapp.com/"  # Final generated base URL
